...
"""Which country a bounding box falls in, from a table that ships with the plugin."""
...
def _contains(box: tuple, west: float, south: float, east: float, north: float) -> bool:
    return (box[1] - _SLACK_DEGREES <= west and box[2] - _SLACK_DEGREES <= south
            and box[3] + _SLACK_DEGREES >= east and box[4] + _SLACK_DEGREES >= north)


def region_of(bbox) -> str | None:
    """The name of the smallest country box that contains ``bbox``, or None."""
    ...
    if not (-180.0 <= west <= east <= 180.0 and -90.0 <= south <= north <= 90.0):
        return None
    best = None
    for box in COUNTRY_BOXES.values():
        if box[3] - box[1] >= _ANTIMERIDIAN_SPAN:
            continue
        if _contains(box, west, south, east, north) and (best is None or _area(box) < _area(best)):
            best = box
